...
class Car:
        ...
        if(msg.topic == "vanetza/out/denm" and self.wantToCharge):
            ...
            if(getCauseCode(message) == event["parkStatus"] and not self.normalSlotReserved and not self.chargerSlotReserved):
                ...
                if(getSubCauseCode(message) == event["parkWithChargerPlace"]):
                    ...
                    result = self.sio.call('send_message', self.sendMessage("Send denm reserveSlotCharger to rsu" + str(getId(message))), to=self.sid)
                    ...
                    result = self.sio.call('send_message', self.sendMessage("Send denm reserveSlotNormal to rsu" + str(getId(message))), to=self.sid)
    ...
    def leaveThePark(self, coords_json):
        ...
        self.mqttc.publish("vanetza/in/denm", json.dumps(self.denm))
        result = self.sio.call('send_message', self.sendMessage("Denm exitPark"), to=self.sid)
        self.parkLatitude = 0
        self.parkLongitude = 0
        self.parkId = None
        self.chargerSlotReserved = False
        ...
        result = self.sio.call(
            'send_coords', self.sendLocation(), to=self.sid)
    ...
    def run(self, a, coords_json):
        ...
        while True:
            ...
            if(self.battery <= 250): #and not self.wantToCharge):
                ...
                self.mqttc.publish("vanetza/in/denm", json.dumps(self.denm))
                result = self.sio.call('send_message', self.sendMessage("Denm batteryStatus"), to=self.sid)
            result = self.sio.call('send_coords', self.sendLocation(), to=self.sid)
            time.sleep(0.2)
